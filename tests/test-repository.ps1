...
if ($embedded.Count -ne ($lifecycleLines.Count + 1)) { throw 'Embedded lifecycle line count differs from source script.' }
for ($i = 0; $i -lt $lifecycleLines.Count; $i++) {
    if ([string]$embedded[$i] -cne [string]$lifecycleLines[$i]) { throw "Embedded lifecycle differs from source at line $($i + 1)." }
}
