...
The readiness probe reads the rendered UI after the asynchronous health response, excluding text hidden by CSS visibility or zero opacity. It requires either usable provider setup controls or the main window's editable composer; an unconfigured provider is allowed because account setup is user-controlled. Visible loading/error states and disabled or covered controls do not pass. Its polling loop awaits the resolved boolean through Playwright `evaluate`, with a deadline that also covers stalled IPC. The smoke test validates the UI before and after the screenshot and retains both rendered text and raw DOM text for diagnosis.
