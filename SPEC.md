...
The readiness probe reads the full visible body after the asynchronous health response. Its polling loop awaits the resolved boolean through Playwright `evaluate`, with a deadline that also covers stalled IPC. The smoke test also rejects loading or error text captured before and after the screenshot, so a UI transition after a successful poll cannot pass validation with incomplete evidence.
